...
/// <summary>
/// Класс, описывающий пул для пуль
/// </summary>
public class PoolOfBullets : Pool 
{
    /// <summary>
    ...
    public new void InstantiateObject(PoolObjectParameters _createParameter)
    {
        ...
        if (_poolOfDisabled.Count == 0)
        {
            ...
            bulletController = newItem.GetComponent<BulletController>();

            //Добавляем созданный объект в пул активных элементов
            _poolOfEnabled.Add(bulletController);

            //Подписываемся на событие отключение только что созданного объекта
            bulletController.ObjectDisabledEvent.AddListener(OnDisabledObj);
        }
        ...
        else
        {
            ...
            bulletController = (BulletController)_poolOfEnabled[_poolOfEnabled.Count - 1];
        }
        //Включить объект
